...
function sourceBadge(source) {
  ...
  return `<span style="background:${color};color:#fff;padding:2px 7px;border-radius:3px;font-size:11px;font-weight:700;letter-spacing:0.5px;font-family:monospace;">${source}</span>`;
}
...
function renderItem(item) {
  ...
  return `
    <div style="background:#fff;border:1px solid #e8e8e8;border-radius:6px;padding:20px;margin-bottom:16px;">
      ...
      <p style="margin:8px 0;font-size:14px;color:#444;line-height:1.6;">${item.summary}</p>
      <div style="background:#f4f7fb;border-radius:4px;padding:10px 14px;margin-top:10px;">
        ...
        <p style="margin:4px 0 0;font-size:13px;color:#333;">${item.actionRequired}</p>
      </div>
      <div style="margin-top:10px;">
        <a href="${item.url}" style="font-size:12px;color:#2563eb;">Read full advisory →</a>
      </div>
    </div>`;
}
...
<body style="margin:0;padding:0;background:#f0f2f5;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
  <div style="max-width:680px;margin:24px auto;background:#f0f2f5;">
    ...
    <div style="background:#1a1a2e;padding:24px 32px;border-radius:8px 8px 0 0;">
      <div style="display:flex;align-items:center;gap:12px;">
        <div>
          ...
          <p style="margin:4px 0 0;font-size:13px;color:#8fa3bf;">${date}</p>
        </div>
      </div>
    </div>
    ...
    <div style="background:#16213e;padding:20px 32px;">
      ...
      <p style="margin:0;font-size:15px;color:#e2e8f0;line-height:1.7;">${newsletter.editorsTake}</p>
    </div>
    ...
    <div style="padding:24px 32px 8px;background:#f0f2f5;">
      ...
      <div style="background:#e8f0fe;border-left:4px solid #2563eb;padding:16px 20px;border-radius:0 6px 6px 0;margin-bottom:24px;">
        ...
        <p style="margin:6px 0 0;font-size:14px;color:#333;">${newsletter.closingNote}</p>
      </div>
    </div>
    ...
    <div style="background:#1a1a2e;padding:20px 32px;border-radius:0 0 8px 8px;text-align:center;">
      ...
      <p style="margin:0;font-size:12px;">
        <a href="${appUrl}" style="color:#60a5fa;text-decoration:none;">Manage Subscription</a>
        &nbsp;·&nbsp;
        <a href="${unsubscribeUrl}" style="color:#60a5fa;text-decoration:none;">Unsubscribe</a>
      </p>
      <p style="margin:12px 0 0;font-size:11px;color:#4a5568;">
        FinCompliance Monitor · Not legal or compliance advice · Always consult your legal counsel.
      </p>
    </div>
...
function buildText(newsletter) {
  ...
  for (const section of newsletter.sections) {
    ...
    for (const item of section.items) {
      text += `\n${item.urgent ? '⚠️ URGENT: ' : ''}[${item.source}] ${item.title}\n`;
      text += `${item.summary}\n`;
